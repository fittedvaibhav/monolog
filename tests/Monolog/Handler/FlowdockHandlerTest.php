...
class FlowdockHandlerTest extends TestCase
{
    ...
    private function createHandler($token = 'myToken')
    {
        ...
        $this->handler = $this->getMockBuilder('Fitted\Monolog\Handler\FlowdockHandler')
            ->setConstructorArgs($constructorArgs)
            ->onlyMethods(['fsockopen', 'streamSetTimeout', 'closeSocket'])
            ->getMock();

        $reflectionProperty = new \ReflectionProperty('Fitted\Monolog\Handler\SocketHandler', 'connectionString');
